...
public interface ChatRepository extends JpaRepository<Chat, Long> {
    ...
    @Query("SELECT c FROM Chat c WHERE c.userA.id = :userId OR c.userB.id = :userId ORDER BY c.lastMessageAt DESC")
    List<Chat> findAllByUserIdOrderByRecent(@Param("userId") Long userId);
}
